...
class RiverbedFeatureExtractorModel(nn.Module):
 ...
 def generate(self, gen_model, gen_tokenizer, target=None, *args,
   ...
   for mlength in range(num_return_sequences, end_length, num_words_per_step):
     ...
     embedding_array =  [a[2] for a in text_array2]
     ...
     if mlength < end_length:
         for i in range(len(text_array2):
           ...
           if target is not None:
             embed_array[i] = target + embed_array[i] + torch.mean(self.get_embedding_by_idx([a['id'] for a in self.search(sent, limit=5)]))
